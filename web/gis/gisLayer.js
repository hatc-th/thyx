...
/*页面初始化时的图层,飞行员*/
var pilotInitLayer = ['T_AD','T_NV','T_PA','T_DA','T_RA','T_TCA'];

/*页面初始化时的图层,空军*/
var airmanInitLayer= ['T_AD','T_NV','T_PA','T_DA','T_RA','T_TCA'];
...
function initContentPane(){
	...
	if(document.getElementById("userroleuicode") != null){
  	...
  	if(panecontent.trim().length == 0){
  		...
  		var cbox = "<input type='checkbox' id='checkAll' onclick='javascript:checkAllBox()'>    全选<br/>" + items.join(' ')
  		dojo.byId("contentPane").innerHTML = cbox;
  	}
}
...
function initFFXLayer(){
	if(featureLayer_3 == undefined || featureLayer_3 == null){
		...
		var a = getLayerId('T_FFX');
		var featureLayer3 = new esri.layers.FeatureLayer(gis_host + "/" + a, { //("地标点")
