...
function executePointQueryTask(geometry) {
	//build query task
	queryTask = new esri.tasks.QueryTask(gis_host + "/44");
...
function pop_init() {
	...
	var featureLayer1 = new esri.layers.FeatureLayer(gis_host + "/44", { //("机场")
	...
	var featureLayer2 = new esri.layers.FeatureLayer(gis_host + "/41", { //("导航台")
		mode : esri.layers.FeatureLayer.MODE_ONDEMAND,
		//mode : esri.layers.FeatureLayer.MODE_SELECTION,
		infoTemplate : template,
		outFields : ["*"]
...
function getTextTitle(graphic) {
	closeDialog();
	var content;
	if (graphic.getLayer().layerId == 44) {
		content = graphic.attributes.AD_NAME;
	} else if (graphic.getLayer().layerId == 41) {
		content = graphic.attributes.NV_NAME;
	} else if (graphic.getLayer().layerId == 43) {
...
function getTextContent(graphic) {
	...
	if (graphic.getLayer().layerId == 44) {//机场显示信息.
		adType = 'AD';//机场时均为AD.
		...
		content += "<a onclick=showWeatherInfo('"+icao_code+"')><img src='images/gis/airportAerial.PNG'>机场常规气象信息</a><br/>";
	} else if (graphic.getLayer().layerId == 41) {//导航台显示信息
	...
	} else if (graphic.getLayer().layerId == 43) {//地标点显示信息.
